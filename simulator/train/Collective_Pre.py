        ...
        while self.RealExpTime <= EndTime:
            ...
            for idx in seq:
                ...
                for vehicle in cluster.IdleVehicles:
                    ...
                    PreList2 = [0, 0, 0, 0, 0, 0, 0, 0, 0]
                    PreList2[4] = self.NaivePre[cluster.ID][cluster.ID]
                    for neighbor in cluster.Neighbor:
                        PreList2[self.Getidx(cluster.ID, neighbor.ID)] = self.NaivePre[cluster.ID][neighbor.ID]
                    pre_idxs2 = np.array(PreList2)
                    idxs2 = pre_idxs2.argsort()
                    s2 = 0
                    for i in idxs2:
                        pre_idxs2[i] = s2
                        s2 += 1
                    idxs2 = idxs2[::-1]
                    pre_idxs2 = minmax_scale(pre_idxs2, (0, 1))
                    ...
                    new_state = GetStateFunction(self, vehicle.ID, cluster, pre_idxs2)

                    reward = RewardFunction(self, vehicle, cluster, pre_idxs2, action)
...
save_path = os.path.join("../", "Models","Pre_baseline", "_", "saves_episode", "VeRL0-" + args.name)
