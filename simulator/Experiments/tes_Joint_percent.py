...
def TEST(self, a, reject_rate, avg_wait, pre_reject, num_disp2, idxx, reject_rate_h, idle_taxi_h,
         order_num_h, reward_h, num_pre_reject, AOV, disp_cost, incentives):
    actor = Actor().to(device)
    actor_name = 'episode_2110actor'
    ...
    while self.RealExpTime <= EndTime:
        ...
        for idx in seq:
            ...
            for vehicle in cluster.IdleVehicles:
                ...
                inc=move(self, vehicle, action, cluster, pre_idxs, idxs)
                incentive+=inc
                num_disp[action] -= 1
                if vehicle.Cluster==cluster:
                    self.StayNum+=1
    ...
    print(idxx)
    num_disp2[idxx] = self.DispatchNum - self.StayNum
