...
def TEST(self, a, reject_rate, avg_wait, pre_reject, num_disp, idxx, reject_rate_h, idle_taxi_h,
    ...
    actor_name = 'episode_2970actor'
    actor_path = os.path.join(save_path2, actor_name)
    ...
    while self.RealExpTime <= EndTime:
        ...
        for idx in seq:
            ...
            for i_batch in range(batch_num):
                ...
                joint_action = DispatchFunction(self, state, actor, 1000000,0)
                if i_batch == batch_num - 1:
                ...
                for v_idx in range(BATCH_SIZE):
                    ...
                    inc=move(self, vehicle, act, cluster, pre_idxs_batch[v_idx], idxs_batch[v_idx])
                    incentive += inc
                    if vehicle.Cluster==cluster:
                        self.StayNum+=1
...
save_path2 = os.path.join("../","Models","A2C_joint_diff_7", "_", "saves_episode", "VeRL0-" + args.name)
